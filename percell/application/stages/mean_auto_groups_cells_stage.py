"""
MeanAutoGroupsCellsStage - Application Layer Stage

Groups extracted cells into automatically determined bins using GMM clustering
based on mean fluorescence intensity per cell.
"""
...
class MeanAutoGroupsCellsStage(StageBase):
    """
    Mean Auto Groups Cells Stage

    Groups extracted cells into automatically determined bins based on
    mean fluorescence intensity per cell using Gaussian Mixture Model (GMM)
    clustering with BIC for optimal cluster selection.

    Unlike AUC-based grouping which uses total pixel intensity (favoring
    larger cells), this method uses mean intensity per pixel, making it
    independent of cell size.
    """

    def __init__(self, config, logger, stage_name="mean_auto_groups_cells"):
        super().__init__(config, logger, stage_name)

    def validate_inputs(self, **kwargs) -> bool:
        """Validate inputs for mean auto groups cells stage."""
    ...
    def run(self, **kwargs) -> bool:
        """Run the mean auto groups cells stage."""
        try:
            self.logger.info("Starting Mean Auto Groups Cells Stage")
            ...
            # Group cells using GMM auto-clustering on mean intensity
            self.logger.info(
                f"Auto-grouping cells by mean intensity "
            ...
            ok_group = _group_cells_mean_auto(
            ...
            if not ok_group:
                self.logger.error("Failed to auto-group cells by mean intensity")
                return False
            self.logger.info(
                "Cells auto-grouped by mean intensity successfully"
            )

            return True

        except Exception as e:
            self.logger.error(f"Error in Mean Auto Groups Cells Stage: {e}")
            return False
