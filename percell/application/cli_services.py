...
def show_menu(ui: UserInterfacePort, args: argparse.Namespace) -> Optional[argparse.Namespace]:
    ...
    ui.info(colorize("3. Data Selection (conditions, regions, timepoints, channels)", Colors.reset))
    ui.info(colorize("4. Single-cell Segmentation (Cellpose)", Colors.green))
    ui.info(colorize("5. Process Single-cell Data (tracking, resizing, extraction, grouping)", Colors.green))
    ui.info(colorize("6. Threshold Grouped Cells (interactive ImageJ thresholding)", Colors.green))
    ui.info(colorize("7. Measure Cell Area (measure areas from single-cell ROIs)", Colors.green))
    ui.info(colorize("8. Analysis (combine masks, create cell masks, export results)", Colors.green))
    ui.info(colorize("9. Cleanup (empty cells and masks directories, preserves grouped/combined data)", Colors.reset))
    ui.info(colorize("10. Advanced Workflow Builder (custom sequence of steps)", Colors.magenta))
